"""ربات کاریابی همراکار - بله - نسخه کامل"""
...
async def on_msg(s, msg):
    ...
    if text in ("🔙 بازگشت به منو", "🏠 منو اصلی"):
        clear(uid)
    ...
    if state==ER_NAME:
        ...
        await api.send_message(s, cid, "🏭 صنعت شرکت:",
                               paginate(INDUSTRIES, [], "ind", 0, cols=1))
    elif state==ER_PHONE:
        ...
        try: dt(uid)["job_salary"]=int(text) if text.isdigit() else 0
        except: dt(uid)["job_salary"]=0
        set_st(uid, JOB_CAT)
...
async def on_cb(s, cb):
    ...
    if data.startswith("prov2:"):
        ...
        if val=="DONE":
            set_st(uid, JS_CATS)
            sel=[]
            await api.send_message(s, cid, "🏷 دسته شغلی (حداکثر 3):", paginate(CATEGORIES, sel, "jscat", 0))
            return
    ...
    if data.startswith("jscat:"):
        ...
        if val in sel: sel.remove(val)
            ...
            await api.answer_cb(s, cbid, "حداکثر 3 دسته!", True)
            return
    ...
    # حقوق
    if data.startswith("jssalary:"):
        try: dt(uid)["js_salary"]=int(text) if text and text.isdigit() else 0
        except: dt(uid)["js_salary"]=0
        set_st(uid, JS_DOB)
        await api.send_message(s, cid, "📅 تاریخ تولد (مثال: 1375/03/15 یا سن مثل 25):", reply_kb([["🔙 بازگشت به منو"]]))
        return
    ...
    if data.startswith("admreq:"):
        ...
        db.approve_resume_request(req_id)
    ...
    if data=="cr:yes":
        ...
        db.upsert_user(cid, role=new, emp_name=None, emp_company=None)
        clear(uid)
        if new=="employer":
            set_st(uid, ER_NAME)
            await api.send_message(s, cid, "✅ *کارفرما*\n\nنام و نام خانوادگی:", reply_kb([["🔙 بازگشت به منو"]]))
...
async def cmd_profile(s, cid):
    ...
    if user["role"]=="employer":
        ...
        text=(f"👤 *پروفایل کارجو*\n\n"
              ...
              f"⭐ امتیاز: {user['js_rating'] or 0:.1f}\n"
              f"📅 {user['reg_date'] or '—'}")
    await api.send_message(s, cid, text, menu_for(user))
...
async def start_job(s, cid, uid):
    ...
    if not user or user["role"]!="employer":
        ...
        return
    if not (user["emp_address"] or user["emp_email"]):
        await api.send_message(s, cid,
    ...
    await api.send_message(s, cid, "💼 *ثبت آگهی*\n\nعنوان شغل:",
                           reply_kb([["🔙 بازگشت به منو"]]))

async def finalize_job(s, cid, uid):
    d=dt(uid)
    job_id=db.create_job(cid,
        title=d.get("job_title"), emp_type=d.get("job_emp_type"),
        location=d.get("job_location"), salary=d.get("job_salary"),
...
async def my_jobs(s, cid):
    ...
    jobs=db.get_employer_jobs(cid)
    if not jobs:
        await api.send_message(s, cid, "هیچ آگهی‌ای ندارید.", menu_for(user))
        return
    await api.send_message(s, cid, f"📋 *{len(jobs)} آگهی شما:*")
    for job in jobs:
...
async def do_search(s, cid, uid):
    d=dt(uid)
    jobs=db.search_jobs(category=d.get("search_cat"))
    clear(uid)
    user=db.get_user(cid)
    if not jobs:
...
async def start_resume(s, cid, uid):
    user=db.get_user(cid)
    if not user or user["role"]!="job_seeker": return
    if not (user["js_name"] and user["js_phone"]):
        await api.send_message(s, cid, "⚠️ لطفاً ابتدا پروفایل را تکمیل کنید.")
        return
    clear(uid)
    set_st(uid, RESUME_JOB)
    await api.send_message(s, cid, "🔍 شماره آگهی را وارد کنید (مثال: 5):",
                           reply_kb([["🔙 بازگشت به منو"]]))
...
async def handle_job_code(s, cid, uid, code):
    ...
    job=db.get_job(job_id)
    if not job or job["status"]!="active" or not job["admin_approved"]:
        await api.send_message(s, cid, "❌ آگهی یافت نشد.")
        return
    app=db.create_application(job_id, cid)
    clear(uid)
    user=db.get_user(cid)
    await api.send_message(s, cid,
        f"✅ *رزومه ارسال شد!*\n\n"
...
async def my_requests(s, cid):
    ...
    apps=[]
    with db._conn() as c:
        apps=c.execute("SELECT a.*, j.title FROM applications a "
                       "JOIN jobs j ON a.job_id=j.job_id "
                       "WHERE a.seeker_cid=?", (cid,)).fetchall()
    if not apps:
...
async def search_seeker(s, cid):
    ...
    if not (user["emp_address"] or user["emp_email"]):
        await api.send_message(s, cid,
            "⚠️ لطفاً ابتدا پروفایل را تکمیل کنید.")
        return
    with db._conn() as c:
        seekers=c.execute("SELECT COUNT(*) FROM users WHERE role='job_seeker'").fetchone()[0]
    await api.send_message(s, cid,
        f"🔎 *جستجوی کارجو*\n\n"
        f"{seekers} کارجو در سیستم وجود دارد.\n\n"
        f"ویژگی جستجو هنوز فعال نیست.\n\n"
        f"درخواست رزومه از طریق درخواست‌های رزومه",
        menu_for(user))
...
async def adm_reqs(s, cid):
    if cid not in ADMIN_IDS: return
    await api.send_message(s, cid, "📩 درخواست‌های رزومه", adm_menu())

async def notify_seekers_job(s, job):
    with db._conn() as c:
        seekers=c.execute("SELECT chat_id FROM users WHERE role='job_seeker' "
                          "AND js_categories LIKE ?",
                          (f"%{job['category']}%",)).fetchall()
    for row in seekers:
        try:
            await api.send_message(s, row["chat_id"],
                ...
                f"برای ارسال رزومه شماره آگهی {job['job_id']} را وارد کنید.",
                inline([[("ارسال رزومه", f"applyjob:{job['job_id']}")]]))
